...
try
    for i=1:input.num_files
        kaka=fullfile(input.pathname,tmp{i});
        tmp2(:,:,i)=importdata(kaka);
    end
catch
    disp('this version does not support files with different number of measurments on each file. Future will...');
    return
end
...
try %if in first file, there is 10th column, then check for IP or SIP
    ...
    elseif input.ip_flag==1
        ...
        input.ip_num=2;
            for i=1:input.num_files        
                input.d4_ip_data(:,i)=tmp2(:,13,i)./1000; 
%                 input.d4_real_data(:,i)=complex(real_data,ip_data);      
            end
    else
        
        for i=1:input.num_files
            input.d4_real_data(:,i)=tmp2(:,13,i);     
        end    
        
        for i=1:input.num_files
             input.d4_stdev_error(:,i)=tmp2(:,14,i); % Currently on fow DC
        end
    end
catch
...
if input.sip_flag==1 || input.ip_flag==1
    try
         for i=1:input.num_files
             input.d4_stdev_error(:,i)=tmp2(:,15,i); % Currently on fow DC
         end        
    catch
...
%input.Wd=sparse(eye(input.num_files*input.num_mes));
input.Wd=sparse([1:input.num_files*input.num_mes],[1:input.num_files*input.num_mes],ones(input.num_files*input.num_mes,1));
