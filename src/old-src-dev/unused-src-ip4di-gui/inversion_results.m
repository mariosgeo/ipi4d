...
if get(handles.checkbox1,'Value')==1
    ...
    if final.time_lapse_flag==0
        tmp=final.res_param1_vs_it(:,itr);
        tmp=log10(tmp);
    else
...
if final.ip_flag==1
                ...
                tmp1=1000*(final.res_param1_vs_it(:,itr)-final.all_res_model(:,itr))./(final.res_param1_vs_it(:,itr));
end    
if final.sip_flag==1 % Plots for DC and IP
        
    if final.time_lapse_flag==0
        tmp=log10(abs(final.res_param1_vs_it(:,itr)));
        tmp1=1000*atan2(imag(final.res_param1_vs_it(:,itr)),real(final.res_param1_vs_it(:,itr)));   
    else
...
[xxx,yyy]=meshgrid(xnew,ynew);
    ...
    figure
     ...
     contourf(xxx,yyy,ZII,128,'EdgeColor','none');
         handles.axes1=gca;
%    contourf(handles.axes1,xxx,yyy,ZII,17,'EdgeColor','none');
     ...
     if final.ip_flag==0 ;title(handles.axes1,['Iteration ',num2str(itr),' RMS=> ',num2str(final.RMS(itr,2))]);end
...
%     contourf(handles.axes2,xxx,yyy,ZII,17,'EdgeColor','none');
     ...
     if final.sip_flag==1; title(handles.axes2,['Phase RMS=> ',num2str(final.RMS(itr,2))]);end
...
% %         end
         text7=cellstr(['RMS error :',num2str(final.RMS(final.itn,2))]);
...
tmp=(final.res_param1_vs_it(:,final.itn))./final.bgr_param; % Only for the last iteration
...
if final.time_lapse_flag==0 
    if final.dc_flag==1
        tmp=[final.param_x -final.param_y final.res_param1_vs_it(:,final.itn) log10(final.res_param1_vs_it(:,final.itn))];
    ...
    elseif final.sip_flag==1
        tmp=[final.param_x -final.param_y abs(final.res_param1_vs_it(:,final.itn))...
            abs(final.res_param1_vs_it(:,final.itn) ) ...
            1000*atan2(  imag(final.res_param1_vs_it(:,final.itn)),  real(final.res_param1_vs_it(:,final.itn))  )   ];
    end
else
    if final.dc_flag==1
