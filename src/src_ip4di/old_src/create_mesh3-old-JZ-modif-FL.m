...
if input.reuse_mesh==1
   mesh=load(input.file_mesh);
...
%%FL: added by J. Zhou (not in git version)
%if input.sip_flag==1
%   mesh.mean_res=10+0.1*1i;     
%%  mesh.mean_res=(mean(10.^(log10(input.real_data))));
%   mesh.mean_res=10^(log10(abs(mesh.mean_res)))*exp(10^(log10(angle(mesh.mean_res)))*1j);
%end
...
try
    save(input.file_mesh, '-struct', 'mesh');
catch
...
% DEFINE BOUNDARIES
%scale=35.5*mesh.probe_spacing/4;    % /!\ HERE IT IS 4, BEFORE IT IS 2???
%left_boundary=min(mesh.add_x_points)-mesh.probe_spacing*scale;
left_boundary=min(mesh.add_x_points)-input.pml_width;
%right_boundary=max(mesh.add_x_points)+mesh.probe_spacing*scale;
right_boundary=max(mesh.add_x_points)+input.pml_width;
%bottom=max(mesh.depth_n)+mesh.probe_spacing*scale;
bottom=max(mesh.depth_n)+input.pml_width;
...
% DEFINE BOUNDARIES
%scale=35.5*mesh.probe_spacing/2;    % /!\ HERE IT IS 2, BELOW IT IS 4???
%left_boundary=min(mesh.add_x_points)-mesh.probe_spacing*scale;
left_boundary=min(mesh.add_x_points)-input.pml_width;
%right_boundary=max(mesh.add_x_points)+mesh.probe_spacing*scale;
right_boundary=max(mesh.add_x_points)+input.pml_width;
...
% call anaglyfo function (what's the point??)
anaglyfo;
...
%FL: plot locations of all quadripoles
...
%            mesh.mes_nodes(i)=j;
%%FL DEBUG
%disp(sprintf('COORDINATES FOUND FOR ELECTRODE NB %d, j = %d', i, j));
%        end

%%Algo 2: find closest node to electrode location
    ...
    %Algo 2: allocate mes_node to closest node
