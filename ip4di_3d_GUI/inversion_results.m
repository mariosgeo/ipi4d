...
global final
        global XI YI ZI  max_x max_y min_x min_y max_z min_z tmp_x tmp_y tmp_z
global map cmap itr
...
slice(handles.axes1,xxx,yyy,-zzz,vi,[],[],[-unique(final.param_z)]);
shading (handles.axes1,'flat');
title(handles.axes1,'Amplitude');    
    ...
    ylabel(handles.axes1,'Distance (m)');
    zlabel(handles.axes1,'Distance (m)');
     xlim(handles.axes1,[min_x max_x]);
     ylim(handles.axes1,[min_y max_y]);
     zlim(handles.axes1,[-max_z -min_z]);
...
FF=TriScatteredInterp(final.param_x,final.param_y,final.param_z,tmp1)  ;
vi=FF(xxx,yyy,zzz);
slice(handles.axes2,xxx,yyy,-zzz,vi,[],[],[-unique(final.param_z)]);
 title(handles.axes2,'Phase');    
    

     colorbar('peer',handles.axes2);
     colormap(final_color);
     shading (handles.axes2,'flat');
    
    axis (handles.axes2,'equal');
    xlabel(handles.axes2,'Distance (m)');
    ylabel(handles.axes2,'Distance (m)');
    zlabel(handles.axes2,'Distance (m)');
     xlim(handles.axes2,[min_x max_x]);
     ylim(handles.axes2,[min_y max_y]);
     zlim(handles.axes2,[-max_z -min_z]);
    ...
    function create_text_mesh(handles)
        ...
        global final XI YI ZI  max_x max_y min_x min_y max_z min_z tmp_x tmp_y tmp_z
...
%         disp_text=[text;text2;text3;text4;text5;text6;text7];
        ...
        tmp_z=zz(2)-zz(1);
        zz=-zz; %Now creative y as negative



        [XI YI ZI]=meshgrid(xx,yy,zz);% and now create the gridata   
...
FF=TriScatteredInterp(final.param_x,final.param_y,final.param_z,final.ACB(:,itr))  ;
vi=FF(xxx,yyy,zzz);

slice(handles.axes1,xxx,yyy,-zzz,vi,[],[],[-unique(final.param_z)]);
 title(handles1.axes1,'Lagrangian Distribution');    
...
FF=TriScatteredInterp(final.param_x,final.param_y,final.param_z,real(final.resolution_1)')  ;
vi=FF(xxx,yyy,zzz);
...
slice(handles.axes1,xxx,yyy,-zzz,vi,[],[],[-unique(final.param_z)]);
vi=FF(xxx,yyy,zzz);
slice(handles.axes1,xxx,yyy,zzz,vi,[],[],[unique(znew)]);
 title(handles1.axes1,'Lagrangian Distribution');    
...
if final.time_lapse_flag==0 
    if final.dc_flag==1
        tmp=[final.param_x final.param_y -final.param_z final.res_param1(:,final.itn) log10(final.res_param1(:,final.itn))];
    elseif final.ip_flag==1
        tmp=[final.param_x final.param_y -final.param_z final.all_res_model(:,final.itn) log10(final.all_res_model(:,final.itn)) final.chargeb];
    elseif final.sip_flag==1
        tmp=[final.param_x final.param_y -final.param_z abs(final.res_param1(:,final.itn))...
            abs(final.res_param1(:,final.itn) ) ...
            1000*atan2(  imag(final.res_param1(:,final.itn)),  real(final.res_param1(:,final.itn))  )   ];
    end
else
    if final.dc_flag==1
        tmp=[final.param_x final.param_y -final.param_z final.d4_res_param1(:,:,final.itn) log10(final.d4_res_param1(:,:,final.itn))];
    elseif final.ip_flag==1
%         tmp=[final.param_x;-final_param_y;final.all_res_model(:,itn);log10(final.all_res_model(:,itn)),final.chargeb];
    elseif final.sip_flag==1
        tmp=[final.param_x final.param_y -final.param_z abs(final.d4_res_param(:,:,final.itn)) log10(abs((final.d4_res_param(:,:,final.itn)))) 1000*atan(imag(final.d4_res_param(:,:,final.itn)),real(final.d4_res_param(:,:,final.itn)))];
    end
...
FF=TriScatteredInterp(final.param_x,final.param_y,final.param_z,final.bgr_param)  ;
vi=FF(xxx,yyy,zzz);
