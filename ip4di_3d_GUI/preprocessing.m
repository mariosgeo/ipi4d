...
try
    ...
    z=-tmp.data(1:num_param,3,1);
    
 z_unique=unique(z);   
    num_param=length(x);
    
    res_model(:,1)=tmp.data(1:num_param,4,1);
    
    sip_flag=0;
    try %if in first file, there is 10th column, then check for IP or SIP
    imag_model(:,1)=tmp.data(1:num_param,5,1);
    sip_flag=1;
    end
    ...
    for i=1:num_files
        tmp=tmp2(:,1,i);
        res_model(:,i)=tmp.data(1:num_param,4);
       if sip_flag==1; imag_model(:,i)=tmp.data(1:num_param,5);end
    end
        
    
res_model(:,2)=res_model(:,2)+12;
...
for i=1:num_files-1
    ...
    subplot(sip_flag+1,num_files,i);
    FF=TriScatteredInterp(x,y,z,L1);
    vi=FF(xi,yi,zi);

    slice(xi,yi,-zi,vi,[],[],[-z_unique]);
    colormap(cmap);
    colorbar
%     axis equal
    xlim([min(x) max(x)]);
    ...
    if sip_flag==1
       ...
       FF=TriScatteredInterp(x,y,z,L2);
       vi=FF(xi,yi,zi);
slice(xi,yi,-zi,vi,[],[],[-z_unique]);
       colormap(cmap);
       colorbar 
