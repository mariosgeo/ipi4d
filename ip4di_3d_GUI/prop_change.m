...
%     tmp_d.data(tpar,:)=0;
    ...
    z_center=tmp_d.data(1:tpar,3);
    mesh.bgr_param=tmp_d.data(1:tpar,4);
    ...
    if input.sip_flag==1
        tmp=tmp_d.data(1:tpar,5); % this is in mrads
        ...
        mesh.bgr_param=complex(real_part,imag_part);
         ...
         tmp_d=importdata(input.par_nm,' ' ,tpar+2);
         ...
         if input.dc_flag==1
             input.real_data0=tmp_d.data(:,14);  
             input.array_model_data_bgr=tmp_d.data(:,15); 
         elseif input.ip_flag==1
             input.real_data0=tmp_d.data(:,14);  
             input.ip_data0=tmp_d.data(:,15);
             input.array_model_data_bgr=tmp_d.data(:,16); 
             input.ip_array_model_data_bgr=tmp_d.data(:,17);             
         elseif input.sip_flag==1
             input.real_data0=complex(tmp_d.data(:,14),tmp_d.data(:,15));
             input.array_model_data_bgr=complex(tmp_d.data(:,16),tmp_d.data(:,17));
         end
...
% 
%         if(mesh.elem_param(j)==abs(i))  ;mesh.prop(j)=mesh.res_param1(i); end
%     end
% end
