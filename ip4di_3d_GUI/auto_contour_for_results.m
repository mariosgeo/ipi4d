function y=auto_contour_for_results
...
if input.sip_flag==0 % Plots for DC and IP
    ...
    elseif input.ip_flag==1 && ip_cnt==2 &&input.sip_flag==0
        tmp=1000*(mesh.res_param2-mesh.res_final_param)./(mesh.res_param2);
...
%-----------------------Plots----------------------------------------------
    ...
    ZI=TriScatteredInterp(mesh.param_x,-mesh.param_y,tmp);
...
if input.loke_colors==0    
    ...
    title(big_part,['Iteration ',num2str(itr),' RMS=> ',num2str(fem.rms_sum1)]);
    axis (big_part,'equal');
...
%     cla(ip_part);
    ZI=TriScatteredInterp(mesh.param_x,-mesh.param_y,tmp1);
    ZII = ZI(mesh.xxx,mesh.yyy);
