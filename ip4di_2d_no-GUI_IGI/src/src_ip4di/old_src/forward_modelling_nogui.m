%
% THIS PROGRAM EXECUTES IP4DI ROUTINES
% WITHOUT USING THE USER INTERFACE
% FOR THE COMPUTATION OF THE FORWARD PROBLEM
%

% INPUTS:
% - model in ascii format
% xi yi amp_sigma_i phi_sigma_i

% OUTPUTS:
% - variables and data file in IP4DI format
...
input.mes_in='../data/Jieyi/forward_chsip.d'
...
input.reuse_mesh=1
input.mesh_file='test_mesh.mat'

% READ DATA
[input]=read_data(input);

% CREATE MESH
if input.reuse_mesh==1 then
...
function pushbutton1_EditModel   %(input,mesh,first_time_call,pp_plot,ip_plot)
...
% first time call
if 1%first_time_call==1
   
   % read model from input file
   %model=load('../data/Jieyi/model4_chsip_31.6228.mod');
   model=load('../data/Jieyi/model4_chsip.mod');

   % if input file contains amplitude and phase of resistivity
   mag=model(:,3);
   phi=model(:,4);
   mesh.res_param1 = complex (mag.*(cos(phi/1000)) , mag.*(sin(phi/1000)));

   %% if input file contains real and imaginary part of resistivity
   %rho_r=model4_chsip(:,3);
   %rho_i=model4_chsip(:,4);
   %mesh.res_param1 = complex (rho_r + i*rho_i );
   ...
   for i=1:mesh.num_param
      ...
      patch('faces',faces','vertices',tmp4,'facecolor','flat','FaceVertexCData',(1000*(angle(input.bgr_res_param(i))))','edgecolor','k');
      hold on
...
function pushbutton5_recreate_mesh
...
function pushbutton6_dd_pseudo
