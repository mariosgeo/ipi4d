...
import edu.mines.jtk.dsp.*;
 ...
 %FL: don't use JZ's routine (use DH's one in loop below instead)
 ...
 for i2=1:m2
     ...
     for i1=1:m1
         ...
         %FL: don't use JZ's routine anymore, use DH's one instead
         evi = ev(:,k2,k1);
...
% %  (column by column, using DH's local diffusion kernel)
% Id=eye(mesh.num_param,mesh.num_param);           % Identity matrix of size [num_par,num_par]
% mesh.ctc=zeros(mesh.num_param,mesh.num_param);   % init. covariance matrix
% meshs=ones(mesh.m2,mesh.m1);                     % do not apply local scaling to the diffusion tensors
% %meshs=mesh.s;                                    % DO     apply   "      "     "  "      "        "
% scale=1;                                         % do not apply global scaling to the diffusion tensors
% for ipar=1:mesh.num_param
%     % take i-th column of Identity matrix and reshape it in matrix format of model size for Java input
%     col_i = reshape(Id(:,ipar),[mesh.m2,mesh.m1]);   %TEST MARIOS' CONVENTIONS
%     %col_i = reshape(Id(:,ipar),[mesh.m1,mesh.m2]);   %TEST SU CONVENTIONS (DOES NOT WORK)
%
%     % applying G'DG to i-th column of Identity matrix gives the i-th column of Cm^1
%     %                                 applyGDG(Tensors2,float[][],float,float[][])
%     Cm1_i = IGICovariance.pkgApplyGDG.applyGDG(mesh.et2,col_i,scale,meshs,stencil);
%     mesh.ctc(:,ipar) = Cm1_i(:);
% end
