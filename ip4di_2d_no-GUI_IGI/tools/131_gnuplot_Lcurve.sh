...
do
 ...
 lagrn2=$(awk <$file_in "NR==$nl {print \$8}")
 ...
 if [ ! $C_M == "NaN" ] && [ ! $C_D == "NaN" ]; then
 #- do the following only if values are not NaN
   ...
   #- customize location of labels
   if [ $il -eq 202 ] || [ $il -eq 212 ] ; then
   ...
   echo "Skip NaN values."
 fi   #-- end loop if NaN

 #-- update line nb for reading in info_vs_it.txt
 il=$(($il+1))
