...
//!\\ BELOW IN PROGRESS...
      ...
      but WARNING: there is dubious source code in WD3_NTFS
      (the soure code stored LOCALLY ON MB PRO stands as the stable version)

  - modify src/java-src/IGICovariance/pkgApplyGDG.java to specify stencil used for the calculation
    of derivatives in LocalDiffusionKernel (hard-coded for the moment, should be passed as
    argument on the long term, unless a specific stencil turns out to be fine).
  (FL: not done finally...)

  - modify src/src-IGI/initcm2.m according to the discussion with Elias Arias (CWP):
    - introduce anisotropic Gaussian windows for the smoothing of structure tensors T.
